 Serial Number & Barcode Generator

A Python-based system for generating unique serial numbers, creating barcodes, and exporting data into Excel and printable PDF label sheets.

This project is designed for asset tracking, inventory management, and equipment labeling in a simple and efficient way

 Features
Generate unique serial numbers with custom prefix (e.g., WGZ-XXXXXXXX)
Prevent duplicate serial numbers automatically
Generate Code128 barcodes for each serial number
Export serial numbers to Excel file (.xlsx)
Create printable A4 PDF label sheets
Organized output folder for all generated files
Easy to customize and extend
 Technologies Used
Python 3
random & string (for serial generation)
python-barcode (barcode generation)
openpyxl (Excel export)
reportlab (PDF generation)
os (file management)
